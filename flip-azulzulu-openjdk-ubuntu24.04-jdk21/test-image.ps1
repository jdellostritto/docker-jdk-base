...
Write-Host "`n[Test 2] Javac" -ForegroundColor Yellow
...
docker run --rm $ImageTag sh -c "cat > T.java << 'EOF'`npublic class T{public static void main(String[]a){System.out.println(\"OK\");}`n}EOF`njavac T.java`njava T" 2>&1
Write-Host "OK" -ForegroundColor Green
...
Write-Host "`n[Test 6] Image Size" -ForegroundColor Yellow
docker images $ImageTag
Write-Host "OK" -ForegroundColor Green

Write-Host "`n[Test 7] Environment" -ForegroundColor Yellow
docker run --rm $ImageTag sh -c "echo JAVA_HOME=`$JAVA_HOME" 2>&1
Write-Host "OK" -ForegroundColor Green
